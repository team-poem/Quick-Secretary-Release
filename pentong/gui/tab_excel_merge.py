...
from core.excel_merge import MergeMode, merge_excel_files


class ExcelMergeTab(ttk.Frame):
    """엑셀 취합 탭 — 여러 xlsx 파일을 하나로 합친다."""
    ...
    def _build_ui(self) -> None:
        ...
        # 취합 방식 선택
        mode_frame = ttk.LabelFrame(self, text="취합 방식")
        mode_frame.grid(row=2, column=0, sticky="ew", padx=10, pady=4)

        ttk.Radiobutton(
            mode_frame,
            text="각 파일을 별도 시트로 (서식 유지)",
            variable=self._mode_var,
            value="sheet",
        ).pack(anchor="w", padx=10, pady=2)
        ttk.Radiobutton(
            mode_frame,
            text="모든 행을 하나의 시트에 연결 (헤더 중복 제거)",
            variable=self._mode_var,
            value="row",
        ).pack(anchor="w", padx=10, pady=2)
    ...
    def _run(self) -> None:
        if not self._files:
            messagebox.showwarning("파일 없음", "취합할 xlsx 파일을 추가하세요.")
            return
        ...
        mode: MergeMode = self._mode_var.get()  # type: ignore[assignment]
        ...
        def worker() -> None:
            try:
                ...
                merge_excel_files(files, out, mode=mode, progress_cb=on_progress)
